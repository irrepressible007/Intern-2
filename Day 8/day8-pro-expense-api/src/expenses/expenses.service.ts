...
export class ExpensesService {
  ...
  private buildFilter(queryDto: QueryExpenseDto, userId: string): FilterQuery<ExpenseDocument> {
    ...
    const filter: FilterQuery<ExpenseDocument> = { userId, isDeleted: false };
  ...
  async create(createExpenseDto: CreateExpenseDto, userId: string): Promise<Expense> {
    ...
    if (!category) throw new BadRequestException('Category not found or you do not own it.');
    if ((category as any).isDeleted) throw new BadRequestException('Cannot add expense to a deleted category.');
    
    const expenseData = { ...createExpenseDto, userId, date: createExpenseDto.date ? new Date(createExpenseDto.date) : new Date() };
  ...
  async attachReceipt(expenseId: string, userId: string, attachReceiptDto: AttachReceiptDto): Promise<PopulatedExpense> {
    ...
    const updatedExpense = await this.expenseModel
      .findOneAndUpdate(
        ...
        { receipt: receipt }, // Set the receipt object (or null)
        { new: true }
      )
      .populate<PopulatedExpense>('categoryId', 'name slug')
      .lean()
  ...
  exportExpenses(queryDto: QueryExpenseDto, userId: string) {
    ...
    const cursor = this.expenseModel
        .find(filter).sort({ date: -1 }).select('title amount date paymentMethod note').lean().cursor(); 
  ...
  async update(id: string, updateExpenseDto: UpdateExpenseDto, userId: string): Promise<PopulatedExpense> {
    if (updateExpenseDto.categoryId) {
      ...
      if (!category) throw new BadRequestException('New category not found or you do not own it.');
    }
     const updatedExpense = await this.expenseModel
       .findOneAndUpdate(
       ...
       .populate<PopulatedExpense>('categoryId', 'name slug')
       .lean().exec();
     if (!updatedExpense) throw new NotFoundException(`Expense with ID "${id}" not found`);
     return updatedExpense as PopulatedExpense;
  }
  ...
  async remove(id: string, userId: string) {
    const deletedExpense = await this.expenseModel
      ...
      .lean().exec();
    if (!deletedExpense) throw new NotFoundException(`Expense with ID "${id}" not found`);

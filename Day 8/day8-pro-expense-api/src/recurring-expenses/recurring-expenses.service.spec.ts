...
export class RecurringExpensesService {
  ...
  async handleCron() {
    ...
    for (const recurring of expensesToRun) {
      // Use the 'nextRunDate' as the official date for this expense
      const runDate = new Date(recurring.nextRunDate);

      // 2. Idempotency Check (Task 5)
      // Check if an expense for this template on this run date *already exists*
  ...
  // findAll returns a populated, lean object, so we infer the type
  async findAll(userId: string) {
    return this.recurringExpenseModel
      ...
      .sort({ nextRunDate: 1 }) // Show the next-to-run first
      .lean()
      .exec();
  }

  // findOne returns a populated, lean object
  ...
  // remove returns a simple message
  async remove(id: string, userId: string) {
    const deletedExpense = await this.recurringExpenseModel
      .findOneAndUpdate(
        { _id: id, userId, isDeleted: false },
        { isDeleted: true }, // Simple soft delete
        { new: true }
      )
      .lean()
      .exec();
